    ...
    return (
        <div className="min-h-screen bg-background">
                                ...
                                {albums.map((album, idx) => (
                                    <ScrollReveal key={album.id} direction="up" delay={idx * 100}>
                                        <div
                                            ...
                                            className="group cursor-pointer bg-card border border-border rounded-[2.5rem] overflow-hidden hover:border-primary/50 transition-all hover:shadow-xl"
                                        >
                            ...
                            {imagesLoading ? (
                                ...
                                <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                                    {images.map((img, idx) => (
                                        ...
                                        >
                                            <div
                                                className="relative group rounded-2xl overflow-hidden cursor-zoom-in border border-border"
                                ...
                                <div className="py-20 text-center border-2 border-dashed border-border rounded-[3rem]">
                                    ...
                                    <p className="text-xs text-muted-foreground mt-2 italic px-4">Se você acabou de subir as fotos, o sistema pode levar alguns minutos para processar o índice do banco de dados.</p>
                                </div>
                            )}
                        </div>
                    )}
            ...
            {fullscreenImage && (
                ...
                >
                    <button
                        className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white transition-all transform hover:rotate-90"
                        onClick={() => setFullscreenImage(null)}
                    >
                        <X className="w-8 h-8" />
                    </button>
