...
import { ChevronLeft, Calendar, MapPin, Clock, ArrowRight, Loader2 } from 'lucide-react';
                    ...
                    <div className="space-y-12">
                        {eventos.map((evento, idx) => (
                            <ScrollReveal key={evento.id} direction="up" delay={idx * 100}>
                                <div className="group bg-card border border-border rounded-[2.5rem] overflow-hidden flex flex-col md:flex-row hover:border-primary/50 transition-all hover:shadow-xl">
                                    <div className="relative w-full md:w-64 aspect-video md:aspect-square flex-shrink-0 overflow-hidden">
                                        <Image src={evento.coverImage} alt={evento.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                                    </div>
                                    <div className="p-8 flex flex-col justify-between flex-grow">
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-muted-foreground mb-6">
                                                <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-primary/60" />{evento.time}</div>
                                                <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-primary/60" />{evento.location}</div>
                                            </div>
